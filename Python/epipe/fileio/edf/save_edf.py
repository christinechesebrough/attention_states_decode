    ...
    try:
        ...
        for i in ch_idx:
            try:
                ...
                ch_dict = {'label': mne_raw.ch_names[i], 
                           'dimension': 'uV' if mne_raw._orig_units[keys[i]] == 'µV' else mne_raw._orig_units[keys[i]],
                           'sample_rate': sfreq,
                           #'sample_frequency': 64,
                           'physical_min': channels.min(), #pmin,
                           'physical_max': channels.max(), #pmax,
                           'digital_min':  dmin, 
                           'digital_max':  dmax, 
                           'transducer': '', 
                           'prefilter': ''}
        ...
        f.writeSamples(channels,digital=False)
