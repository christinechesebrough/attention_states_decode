...
#%%
    ...
    # ET times are in microseconds
    et_ttl_times = ttls_dict['et_ttl_time']
    et_ttl_times = et_ttl_times /1000000

    #eeg times are in seconds
    eeg_ttl_times = ttls_dict['eeg_ttl_time']
    
    
    et_first = et_ttl_times[0]
    et_last = et_ttl_times[-1]

    eeg_first = eeg_ttl_times[0]
    eeg_last = eeg_ttl_times[-1]
        
    print(f"ET duration: {et_last - et_first:.3f} s")
    print(f"EEG duration: {eeg_last - eeg_first:.3f} s")
    
    # Compute TTL intervals
    et_intervals = np.diff(et_ttl_times)
    eeg_intervals = np.diff(eeg_ttl_times)
    
    min_len = min(len(et_intervals), len(eeg_intervals))
    interval_diff = et_intervals[:min_len] - eeg_intervals[:min_len]
    
    # get indices where interval_diff < .05 s (abs)
    # filter to keep only those indices in eeg_ttl_time and eeg_code
    
    # Plot comparison
    plt.figure(figsize=(14, 6))
    
    plt.subplot(2, 1, 1)
    plt.plot(et_intervals, 'g*-', label='ET TTL intervals')
    plt.plot(eeg_intervals, 'b*-', label='EEG TTL intervals')
    plt.ylabel('Interval duration [s]')
    plt.title('Comparison of TTL Intervals: ET vs EEG')
    plt.legend()
    plt.grid(True)
    ...
    min_len = min(len(et_intervals), len(eeg_intervals))
    interval_diff = et_intervals[:min_len] - eeg_intervals[:min_len]
    plt.plot(interval_diff, 'r.-', label='ET - EEG interval diff')
