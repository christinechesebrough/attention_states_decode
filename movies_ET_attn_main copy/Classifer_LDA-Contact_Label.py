...
#
# = 500 temporary Shrinkage LDA models
#
...
for area in atlas_areas:
    ...
    if subset_by_area:
        print(f"Running LDA model for {atlas}: {area}")
        df = df_all[df_all[atlas] == area].copy()
    else:
        print(f"Running LDA model using all {atlas} areas")
        df = df_all.copy()

    print("=" * 60)
    print("Filtered dataframe shape:", df.shape)
    
    df = df[df["Desikan_Killiany"] != "Out"].copy()
    ...
    mean_cv_balanced_accuracy = np.mean(
        balanced_accuracy_results
    )
    ...
    output_folder = Path(
       f"/media/christine/Samsung/attn_states_decoding/outputs/LDA/{patient_id}"
    )
    ...
    output_file = (
        output_folder
        / f"{patient_id}_lda_contact_attention_results_{attn_scale}_{atlas}_{area}.csv"
    )
    ...
    if subset_by_area:
        # ==========================================================
        # Mean coefficients by frequency for current atlas-area model
        # ==========================================================
        area_frequency_mean_coefficients = (
            results_df
            .groupby([grouping_atlas, "frequency"], dropna=False)["mean_coefficient"]
            .agg(
                mean_coefficient="mean",
        ...
        area_frequency_mean_coefficients = (
            results_df
            .groupby(["frequency"], dropna=False)["mean_coefficient"]
            .agg(
                mean_coefficient="mean",
                mean_abs_coefficient=lambda x: x.abs().mean(),
                coefficient_std="std",
    ...
    area_frequency_mean_coefficients.insert(0, "area", area)
...
# ==========================================================
# Combine coefficient summaries across atlas-area models
# ==========================================================

atlas_coefficient_results_df = pd.concat(atlas_coefficient_results, ignore_index=True)
atlas_coefficient_results_df = atlas_coefficient_results_df.sort_values(["area", "frequency"]).reset_index(drop=True)
...
print(f"\nMean LDA coefficients across all {atlas} area-specific models:")
print(atlas_coefficient_results_df.to_string(index=False))

if subset_by_area:
    coefficient_label = "area_models"
else:
    coefficient_label = "all_areas_model"

atlas_coefficients_file = output_folder / f"{patient_id}_LDA_{atlas}_{coefficient_label}_mean_coefficients_{attn_scale}.csv"
...
atlas_model_results_df = pd.DataFrame(atlas_model_results)
atlas_model_results_df = atlas_model_results_df.sort_values("mean_cv_balanced_accuracy", ascending=False).reset_index(drop=True)

print(f"\nLDA model performance across {atlas} areas:")
print(atlas_model_results_df.to_string(index=False))

atlas_comparison_file = output_folder / f"{patient_id}_LDA_{atlas}_area_comparison_{attn_scale}.csv"
atlas_model_results_df.to_csv(atlas_comparison_file, index=False)
...
if subset_by_area:
    ...
    outdir = f"/media/christine/Samsung/attn_states_decoding/{patient_id}_{model}_{atlas}_bargraphs"
    ...
    ax.set_title(f"{patient_id}: Mean {model} coefficients across {atlas} area-specific models ({attn_scale})")
    ...
    ax.set_title(f"{patient_id}: Mean absolute {model} coefficients across {atlas} area-specific models ({attn_scale})")
    ax.legend(title="Frequency", frameon=False, ncol=5)
    fig.tight_layout()
    ...
    ax.set_xlabel(f"{atlas} parcels")
    ax.set_title(f"{patient_id}: {model} decoding performance across {atlas} parcels ({attn_scale})")
    ...
    outdir = f"/media/christine/Samsung/attn_states_decoding/{patient_id}_{model}_coefs_bargraphs"
    os.makedirs(outdir, exist_ok=True)
    ...
    ax.set_title(f"{patient_id}: Mean {model} coefficients by frequency ({attn_scale})")
    
    fig.tight_layout()
    plt.show()
    
    signed_png = os.path.join(outdir,f"{patient_id}_{model}_mean_coefficients_by_freq_{attn_scale}.png")
    fig.savefig(signed_png, dpi=300, bbox_inches="tight")